...
//Establecer fecha minima de atención mañana
...
tomorrow = yyyy + '-' + mm + '-' + dd;
fechaInput.setAttribute('min', tomorrow);
//Restringir los fines de semana el el input de fecha
fechaInput.addEventListener('input', (e) => {
  let dia = new Date(e.target.value).getUTCDay();

  if ([6, 0].includes(dia)) {
    const fecha = document.getElementById('fecha-container');
    ...
    divMensaje.textContent = 'No Atendemos los Fines de Semana';
    fecha.append(divMensaje);
    e.target.value = '';
    setTimeout(() => {
      divMensaje.remove();
    }, 3000);
...
class UI {
  ...
  imprimirCitas() {
    ...
    //Leer de la BD
    const transaction = DB.transaction('citas').objectStore('citas');
    transaction.openCursor().onsuccess = (e) => {
      const cita = e.target.result;
      if (cita) {
        const cursor = cita.value;
        ...
        btnEditar.onclick = () => cargarEdicion(cursor);
        ...
        //Iterar en todos los elementos del IndexDB
        cita.continue();
      }
    };
  }
...
function crearDB() {
  ...
  crearDB.onupgradeneeded = (e) => {
    const db = e.target.result,
      objectStore = db.createObjectStore('citas', {
        keyPath: 'id',
        autoIncrement: true,
      });
